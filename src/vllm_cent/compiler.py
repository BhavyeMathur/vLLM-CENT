...
#
# Exact-type dispatch is enough while Llama is the only supported family. A
...
def compile_transformer_block(request: CompileRequest) -> CentProgram:
    ...
    # The request's model type chooses the matching compiler. Require the exact
    # type so a subclass with extra, unsupported behavior is not mistaken for
    # an ordinary Llama model.
    if type(request.model) is LlamaModelSpec:
